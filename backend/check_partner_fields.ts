...
async function bootstrap() {
  ...
  const fields = data._embedded?.custom_fields?.filter((f: any) => 
    f.name.toLowerCase().includes('партнер') || 
    f.name.toLowerCase().includes('деятельност') ||
    f.name.toLowerCase().includes('направлени')
  ).map((f: any) => ({id: f.id, name: f.name, enums: f.enums}));
